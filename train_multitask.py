...
Generator = MOE_all_position(dim = 22, num_blocks=[3, 4, 4, 5]).cuda() # 4.3M
# Generator = Restormer(inp_channels=9, out_channels=8, dim = 22, num_blocks=[3, 4, 4, 6]).cuda()
...
def validate_one_epoch(model, datasets, test_minmax, logger, epoch):
    ...
    with torch.no_grad():
        ...
        if psnr[0] > best_psnr_pan:
            torch.save(model.state_dict(), os.path.join(save_dir, 'Best_Pan.pth'))
