...
from model.Model_others import Restormer
from data.dataset import Pansharpening_mat_Dataset, MRI_pre_dataset, NYU_v2_datset, MultiTaskDataset
import numpy as np
import argparse

def parse_args():
    parser = argparse.ArgumentParser(description='Training 9 Tasks with Restormer')
    parser.add_argument('--exp_name', type=str, default='Restormer_9tasks', help='experiment name')
...
def main():
    ...
    Generator = Restormer(dim=24, num_blocks=[3, 4, 4, 6]).cuda()
