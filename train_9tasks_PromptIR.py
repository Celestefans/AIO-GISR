...
from model.Compare.PromptIR import PromptIR_multitasks
from data.dataset import Pansharpening_mat_Dataset, MRI_pre_dataset, NYU_v2_datset, MultiTaskDataset
import numpy as np
import argparse

def parse_args():
    parser = argparse.ArgumentParser(description='Training 9 Tasks with PromptIR')
    parser.add_argument('--exp_name', type=str, default='PromptIR_9tasks', help='experiment name')
...
# Initialize logger
    ...
    Generator = PromptIR_multitasks(dim = 20, num_blocks=[3, 4, 4, 5], decoder=True, prompt_dim=[20,40,104]).cuda()
    ...
    for epoch in range(start_epoch, args.num_epoch + 1):
        ...
        train_dataloader = DataLoader(
            ...
            pin_memory=True,
            persistent_workers=False,  # 关闭persistent_workers
            prefetch_factor=2  # 添加预取因子限制
        )
