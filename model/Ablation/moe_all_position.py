...
##########################################################################
##---------- AMIR -----------------------
class MOE_all_position(nn.Module):
    def __init__(self, 
        inp_channels=1, 
        out_channels=1, 
        dim = 42,
        ...
        super(MOE_all_position, self).__init__()
        ...
        atom_dim = 128
        atom_num = 16 
        self.dict_generator1 = RIN(in_dim=4, atom_num=atom_num, atom_dim=atom_dim) # depth
        self.dict_generator2 = RIN(in_dim=2, atom_num=atom_num, atom_dim=atom_dim) # mri
        self.dict_generator3 = RIN(in_dim=5, atom_num=atom_num, atom_dim=atom_dim) # pan
        ...
        self.spatial_routing_encoder_level4 = Spatial_Routing(atom_dim = atom_dim, dim=int(dim*2**3), ffn_expansion_factor=ffn_expansion_factor)
        self.spatial_routing_decoder_level3 = Spatial_Routing(atom_dim = atom_dim, dim=int(dim*2**2), ffn_expansion_factor=ffn_expansion_factor)
        self.spatial_routing_decoder_level2 = Spatial_Routing(atom_dim = atom_dim, dim=int(dim*2**1), ffn_expansion_factor=ffn_expansion_factor)
        self.spatial_routing_decoder_level1 = Spatial_Routing(atom_dim = atom_dim, dim=int(dim*2**1), ffn_expansion_factor=ffn_expansion_factor)
    ...
    def forward(self, inp_ms, inp_pan): 
        ...
        if C1 + C2 == 4: # depth
            ...
            prompt = self.dict_generator1(inp_img)
        elif C1 + C2 == 2 : # mri 
            inp_enc_level1 = self.patch_embed2(inp_img)
            prompt = self.dict_generator2(inp_img)
        elif C1 + C2 == 5: # pan
            inp_enc_level1 = self.patch_embed3(inp_img)
            prompt = self.dict_generator3(inp_img)
        ...
        inp_enc_level4, loss_tmp = self.spatial_routing_encoder_level4(inp_enc_level4, prompt)
        loss_importance = loss_importance + loss_tmp
        ...
        inp_dec_level3, loss_tmp = self.spatial_routing_decoder_level3(inp_dec_level3, prompt)
        loss_importance = loss_importance + loss_tmp
        ...
        inp_dec_level2, loss_tmp = self.spatial_routing_decoder_level2(inp_dec_level2, prompt)
        loss_importance = loss_importance + loss_tmp
        ...
        inp_dec_level1, loss_tmp = self.spatial_routing_decoder_level1(inp_dec_level1, prompt)
        loss_importance = loss_importance + loss_tmp
...
if __name__ == '__main__':
    ...
    # model = AMIR(dim = 22, num_blocks=[3, 4, 4, 5])
    model = MOE_all_position(dim = 22, num_blocks=[3, 4, 4, 5])
    model.cuda()
    model.eval()

    out_depth = model(depth_1, depth_2)
    out_mri = model(mri_1,mri_2)
    out_pan = model(pan_1, pan_2)
