...
class MOE_IFM(nn.Module):
    ...
    def forward(self, inp_tar, inp_guide): 
        ...
        if C1 + C2 == 4: # depth
            inp_tar_feature, inp_guide_feature = self.ifm(self.inp_tar_embed1(inp_tar), self.inp_guide_embed1(inp_guide))
            ...
            inp_tar_feature, inp_guide_feature = self.ifm(self.inp_tar_embed2(inp_tar), self.inp_guide_embed2(inp_guide))
            inp_enc_level1 = self.inp_feature_fuse(torch.cat([inp_tar_feature, inp_guide_feature], dim=1))
            prompt = self.dict_generator2(inp_img)
        elif C1 + C2 == 5: # pan
            inp_tar_feature, inp_guide_feature = self.ifm(self.inp_tar_embed3(inp_tar), self.inp_guide_embed3(inp_guide))
            inp_enc_level1 = self.inp_feature_fuse(torch.cat([inp_tar_feature, inp_guide_feature], dim=1))
...
if __name__ == '__main__':
    ...
    model = MOE_IFM(dim = 22, num_blocks=[3, 4, 4, 5])
    model.cuda()
    model.eval()
