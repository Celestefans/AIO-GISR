...
class PromptIR_multitasks(nn.Module):
    def __init__(self, 
        inp_channels=3, 
        inp_size = 128,
        out_channels=3, 
        ...
        super(PromptIR_multitasks, self).__init__()
        ...
        if self.decoder:
            self.prompt1 = PromptGenBlock(prompt_dim=prompt_dim[0],prompt_len=5,prompt_size = inp_size//2**1,lin_dim = dim*2**1)
            self.prompt2 = PromptGenBlock(prompt_dim=prompt_dim[1],prompt_len=5,prompt_size = inp_size//2**2,lin_dim = dim*2**2)
            self.prompt3 = PromptGenBlock(prompt_dim=prompt_dim[2],prompt_len=5,prompt_size = inp_size//2**3,lin_dim = dim*2**3)
        ...
        self.reduce_noise_level3 = nn.Conv2d(int(dim*2**3)+prompt_dim[2],int(dim*2**2),kernel_size=1,bias=bias)
...
if __name__ == "__main__":
    ...
    depth_1, depth_2 = depth_1.type(torch.FloatTensor).to(device), depth_2.type(torch.FloatTensor).to(device)
    mri_1, mri_2 = mri_1.type(torch.FloatTensor).to(device), mri_2.type(torch.FloatTensor).to(device)
    pan_1, pan_2 = pan_1.type(torch.FloatTensor).to(device), pan_2.type(torch.FloatTensor).to(device)
    
    model = PromptIR_multitasks(dim = 20, num_blocks=[3, 4, 4, 5], decoder=True, prompt_dim=[20,40,104]).to(device)
    # model.cuda()
    # model.eval()
