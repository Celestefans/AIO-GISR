...
list_val_dataset = [val_depth_dataset_4, val_depth_dataset_8, val_depth_dataset_16, val_pan_dataset_WV4, val_pan_dataset_QB, val_pan_dataset_GF1, val_mri_dataset]  # 0,1,2,3,4,5,6


############ Model, Optimizer and Scheduler ############
Generator = AMIR(dim = 22, num_blocks=[3, 4, 4, 5]).cuda()
...
rebalance_L1 = Rebalance_L1(num_tasks=7, momentum=0.9, T=10).cuda()
...
############ Training Function ############
def train_one_epoch(epoch, dataloader, model, optimizer, loss_fn, last_losses, min_losses):
    model.train()
    ...
    for train_data_lists in pbar:
        for task_id, data_list in enumerate(train_data_lists):
            ...
            optimizer.zero_grad()
            restored, loss_moe = model(inp_lr, inp_guide)
            
            # 计算rebalance loss
            loss_task, raw_loss, loss_grads, weights = rebalance_L1(
                restored, inp_gt, task_id
            )
            ...
            epoch_losses['task_losses'][task_id].append(raw_loss)
            
            # 更新last_losses
            if len(epoch_losses['task_losses'][task_id]) > 0:
                last_losses[task_id] = np.mean(epoch_losses['task_losses'][task_id])
            
            # 更新min_losses
            min_losses[task_id] = min(min_losses[task_id], raw_loss)
            
            # 记录到tensorboard
            writer.add_scalar(f'Train/Task_{task_id}_Grad', loss_grads[task_id].item(), epoch)
            writer.add_scalar(f'Train/Task_{task_id}_Weight', weights[task_id].item(), epoch)
            
            pbar.set_postfix(
                loss=loss_G.item(),
                task=task_id,
                weight=weights[task_id].item(),
    ...
    return avg_total_loss, avg_task_losses, last_losses, min_losses
...
for epoch in range(1, num_epoch + 1):
    ...
    avg_total_loss, avg_task_losses, last_losses, min_losses = train_one_epoch(
        epoch, train_dataloader, Generator, optimizer_G, rebalance_L1, 
        last_losses, min_losses  # 传入状态
    )
    lr_scheduler_G.step()
    ...
    # if epoch % 1 == 0 :
        metrics, improved = validate_one_epoch(Generator, list_val_dataset, test_minmax, logger, epoch)
        ...
        checkpoint = {
            ...
            'metrics': metrics,
            'loss_grads_ema': rebalance_L1.loss_grads_ema,
            'update_counts': rebalance_L1.update_counts
        }
    ...
    # 监控权重分布
    if epoch % 10 == 0:
        current_weights = rebalance_L1.get_task_weights()
        logger.info(f"Task weights: {current_weights}")
