...
def parse_args():
    parser = argparse.ArgumentParser(description='Training 3 Tasks with CLIP')
    parser.add_argument('--exp_name', type=str, default='MOE_Clip_AIO_GISR_3tasks', help='experiment name')
...
def train_one_epoch(epoch, dataloader, model, optimizer, loss_fn, clip_feat, writer):
    try:
        model.train()
        epoch_losses = {'total': 0, 'task_losses': [[] for _ in range(3)]}
...
def validate_one_epoch(model, datasets, test_minmax, logger, epoch, clip_feat, writer, save_dir, optimizer, scheduler):
    global best_psnr_pan_WV4
    global best_psnr_mri_4x
    global best_rmse_4
    ...
    with torch.no_grad():
        ...
        for dataset_id, dataset in enumerate(datasets):
            ...
            if dataset_id == 0:  # depth estimation task
                for index, (inp_lr, inp_gt, inp_guide) in enumerate(tqdm(val_dataloader, desc='Validating Depth_4')):
...
# Initialize logger
    ...
    best_psnr_pan_WV4 = -float('inf')
    best_psnr_mri_4x = -float('inf')
    ...
    mix_dataset = MultiTaskDataset(depth_dataset_4, mri_dataset_4, pan_dataset_WV4)
    ...
    list_val_dataset = [val_depth_dataset_4, val_mri_dataset_4, val_pan_dataset_WV4]
